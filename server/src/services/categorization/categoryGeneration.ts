...
export async function getLLMCategorySuggestion(
  ...
  try {
    ...
    const filteredCategories = parsed.data.categories.filter(
      (c) => c.name.toLowerCase() !== 'distraction'
    );

    return {
      categories: [
        ...filteredCategories,
        { name: 'Distraction', description: 'Scrolling social media, browsing unrelated content, or idle clicking', color: '#EC4899', emoji: '🎮', isProductive: false },
        { name: 'Friends & Social', description: 'Spending time with friends or socializing in person or online', color: '#A855F7', emoji: '🎉', isProductive: false },
      ],
    };
  } catch (error) {
